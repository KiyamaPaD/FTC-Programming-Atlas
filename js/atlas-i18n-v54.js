...
// v54 · Viewer cleanup + English coverage patch
//
// This small layer runs after atlas-i18n.js v53. It intentionally keeps
// viewer-facing copy separate from editor/admin guidance and fills the last
// dynamic Romanian → English gaps without touching Atlas core state.
...
function translateExtraTextNode(node) {
  ...
  if (!key || !EXTRA_EN.has(key)) return

  const translated = EXTRA_EN.get(key)

  // Empty translations are viewer-only cleanup and are handled separately.
...
function setViewerToolsNote(viewerMode) {
  ...
  if (!note.dataset.atlasV54EditorCopy) {
    note.dataset.atlasV54EditorCopy = note.textContent || ''
  }

  if (!viewerMode) {
    note.textContent = note.dataset.atlasV54EditorCopy
    return
  }
...
function applyTutorialView() {
  ...
  if (!textarea.dataset.atlasV54SourceTutorial) {
    textarea.dataset.atlasV54SourceTutorial = textarea.value || ''
  }

  const editorActive = editorModeActive()

  if (!editorActive) {
    textarea.value = isEnglish()
      ? VIEWER_TUTORIAL_EN
      : VIEWER_TUTORIAL_RO
  ...
  if (isEnglish()) {
    ...
    if (saveButton) {
      ...
      saveButton.title =
        'Switch to RO to edit and save the source tutorial.'
    }

    if (subtitle) {
      subtitle.textContent =
        'Editor guide in English. Switch to RO to modify and save the source tutorial.'
    }
  } else {
    textarea.value = textarea.dataset.atlasV54SourceTutorial || textarea.value
...
function injectStyles() {
  if (document.getElementById('atlasV54Styles')) return

  const style = document.createElement('style')
  style.id = 'atlasV54Styles'
  style.textContent = `
    .atlas-viewer-hidden {
